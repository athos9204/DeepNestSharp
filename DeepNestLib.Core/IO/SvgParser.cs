  ...
  public class SvgParser
  {
    ...
    public static IRawDetail LoadSvg(string path)
    {
      ...
      foreach (var item in doc.Descendants(XName.Get("polygon", ns)))
      {
        ...
        s.AddContour(new LocalContour<XElement>(points.ToList(), new HashSet<XElement> { item }));
      }
    ...
    /// </summary>
    /// <param name="element">The SVG element to polygonify.</param>
    /// <returns>A polygon from the given SVG element in the form of an array of points.</returns>
    /// <exception cref="NotImplementedException">Not many entity types have been implemented.</exception>
